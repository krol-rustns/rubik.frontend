...
export default function EditProperty() {
  ...
  const handleEditProperty = async (values: any) => {
    try {
      ...
      await storage.updateProperty(valuesWithoutCep, cep);

      router.back();
    } catch (error) {
      setPropertyError('Erro ao atualizar imóvel.');
    }
  };
    ...
    <ScrollView style={styles.container}>
      <View style={styles.container}>
        <Text style={styles.header}>
          <Text style={styles.headerHighlight}>Alteração</Text> dos dados do imóvel
        </Text>
        ...
        <Formik
          initialValues={{
            ...
            cep: property?.cep || '',
            tipo: property?.tipo || '',
            qtdQuartos: property?.qtdQuartos || 1,
            qtdBanheiro: property?.qtdBanheiro || 1,
            qtdVagasGaragem: property?.qtdVagasGaragem || 1,
            dataAquisicao: property?.dataAquisicao || new Date().toISOString(),
            registroCartorio: property?.registroCartorio || '',
            inscricaoIptu: property?.inscricaoIptu || '',
            inscricaoCaesb: property?.inscricaoCaesb || '',
            inscricaoNeoenergia: property?.inscricaoNeoenergia || '',
            valorVenal: property?.valorVenal || 1,
          }}
